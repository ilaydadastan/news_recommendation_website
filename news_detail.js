...
const createNewsRow = function (newsListByCategory, colSizeClass, hasContent) {
  let content = "";
  chunk(newsListByCategory, 4).forEach(function(newsList) {
    content += '<div class="row pb-5 mb-4">' + createNewsCategory(newsList, colSizeClass, hasContent) + "</div>";
  })
  return content;

}

const createNewsCategory = function(newsListByCategory, colSizeClass, hasContent) {
  return  newsListByCategory.map(function(news) {return createNewsDiv(news, colSizeClass, hasContent)}).join(' ');
}

const createNewsDiv = function (newsModel, colSizeClass, hasContent) {
  let newsTemplate = [
    ...
    newsModel.title,
    '</a></h5>',
    hasContent ? '<p className="small text-muted font-italic">' + newsModel.content + '</p>' : '',
      '</div>',
    '</div>',
    '</div>'
  ]
...
sendRequest = async function (newsDetail) {
  ...
  response.json().then(data => {
    let content = "";
    if (data.length > 0 ) {
      content += "<h3>Sizin için Önerilerimiz</h3></br>"
    }
    content += createNewsRow(data, "col-lg-3 col-md-6 mb-4 mb-lg-0", false);
    $("#newsRecommend").append(content);
  });
}
$(document).ready(function() {
  ...
  const newsDetail = newsList.find(function(n) {return n.id == newsId});

  let content = createNewsRow([newsDetail], "col-lg-8 col-md-8 mb-8 mb-lg-0", true);
  $("#newsContent").append(content);
  sendRequest(newsDetail);
});